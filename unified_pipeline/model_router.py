...
class ModelRouter:
    def __init__(
        ...
        self.request_timeout     = int(os.getenv("MODEL_REQUEST_TIMEOUT",     request_timeout))
        self.max_retries         = int(os.getenv("MODEL_MAX_RETRIES",         max_retries))
        self.retry_sleep_seconds = int(os.getenv("MODEL_RETRY_SLEEP_SECONDS", retry_sleep_seconds))
        self.max_output_tokens   = int(os.getenv("MODEL_MAX_OUTPUT_TOKENS",   max_output_tokens))
        self.token_parameter     =     os.getenv("MODEL_TOKEN_PARAMETER",     token_parameter)
    ...
    def execute_sql_sqlite(
        ...
        """Execute SQL against an on-disk SQLite file.

        Used for multi-table datasets (F1, Music) where the DB is too
        large to load into a DataFrame.  Returns (result_csv, error).
        """
        if not sql or not sql.strip():
            return "", "empty_generated_sql"
