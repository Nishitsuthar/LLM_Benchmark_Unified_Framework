...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Prompt menu index (1-based) to select non-interactively",
    )
...
def select_prompt_interactive(
    ...
    If *preselect* is given (1-based), skip the interactive prompt.
    """
    ...
    if preselect is not None:
        if 1 <= preselect <= len(options):
            print(f"  Auto-selected: {preselect}")
            label, path = options[preselect - 1]
            return label, path.read_text(encoding="utf-8")
        raise ValueError(
            f"--prompt_num {preselect} out of range (1–{len(options)})"
        )
...
def main() -> None:
    ...
    prompt_label, template = select_prompt_interactive(
        options, preselect=args.prompt_num
    )
    ...
    if mode == "sql_detour" and config.get("data_file"):
    ...
    # For sql_detour against a SQLite file, swap in the scalar evaluator
    # (IMDb uses data_file + TableF1Evaluator; F1/Music use sql_db_file + SqlScalarEvaluator)
    if mode == "sql_detour" and config.get("sql_db_file"):
        from unified_pipeline.evaluators.sql_scalar_evaluator import SqlScalarEvaluator
        evaluator = SqlScalarEvaluator()
    ...
    for i, question in enumerate(
        ...
        try:
            ...
            if mode == "sql_detour":
                ...
                if config.get("sql_db_file"):
                    # SQLite path: F1, Music — multi-table on-disk DB
                    sql_result_csv, sql_exec_error = (
                        ModelRouter.execute_sql_sqlite(
                            generated_sql,
                            config["sql_db_file"],
                        )
                    )
                else:
                    # DataFrame path: IMDb — single-table CSV loaded in memory
                    sql_result_csv, sql_exec_error, _, _ = (
                        router.execute_sql(
                            generated_sql,
                            subset_df,
                        )
            ...
            # Preserve SQL error detail in eval_status when applicable
            eval_status = result.evaluation_status
            if sql_exec_error:
                eval_status = f"sql_error: {sql_exec_error}"
            ...
            delay = float(os.getenv("MODEL_REQUEST_DELAY", 0))
            if delay > 0:
                time.sleep(delay)
